...
n_steps = 20  # number of time steps in our simulation
dt = T / n_steps
S_0 = 50
...
hg = hl.Halton(n_steps)

for no_of_paths in range(10, Max_n_of_p+1, 10):

    present_payoffs = np.zeros(no_of_paths)
    hal_seq = hg.get(no_of_paths)
    epsilon_h = stats.norm.ppf(hal_seq)
    for k in range(no_of_paths):
        price_steps = np.zeros(n_steps)
        price_steps[0] = S_0
        for i in range(1, n_steps):
            price_steps[i] = price_steps[i-1] * np.exp((r - sigma ** 2 / 2) * dt + sigma * epsilon_h[k, i] * np.sqrt(dt))
...
plt.title("Histogram of present value of payoffs in QMC (Halton) Method")
plt.xlabel("Bins")
...
plt.title("Accuracy of Estimation in QMC (halton)")
plt.show()