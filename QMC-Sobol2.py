...
for no_of_paths in range(10, Max_n_of_p+1, 10):
    ...
    for k in range(no_of_paths):
        ...
        price_steps[0] = S_0
        sob = ch.create_sobol_samples(n_steps, 1, seed=k)
        epsilon_s = stats.norm.ppf(sob)
        for i in range(1, n_steps):
            price_steps[i] = price_steps[i-1] * np.exp((r - sigma ** 2 / 2) * dt + sigma * epsilon_s[0, i] * np.sqrt(dt))
